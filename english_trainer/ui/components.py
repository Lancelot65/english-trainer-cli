...
class ModernUI:
    ...
    def main_menu(
        ...
        if show_help:
            ...
            table.add_column(style="", justify="left")

            for key, desc, color in commands:
                table.add_row(f"[{color}]{key}[/]", f"[{color}]{desc}[/]")
            ...
            # Simple prompt without showing all commands
            self.console.print(
                Panel(
                    f"[{self.theme['muted']}]⏎ Nouvel exercice • d Défi quotidien • h pour l'aide • q pour quitter[/]",
    ...
    def theme_menu(self, themes: List[str], current_theme: str) -> None:
        ...
        self.clear()
        self.console.print(
            Panel("[bold]Sélection du Thème[/]", border_style=self.theme["secondary"], box=DOUBLE)
        )
    ...
    def feedback_display(self, evaluation: Dict[str, Any]) -> None:
        ...
        if ideal:
            content.add_row(f"[bold]✅ Traduction idéale:[/]")
        ...
        if suggestions:
            content.add_row(f"[{self.theme['accent']}]🎯 Suggestions d'amélioration:[/]")
            for suggestion in suggestions:
                content.add_row(f"• {suggestion}")

        self.console.print(
            Panel(
                content, title="[bold]📊 Correction[/]", border_style=color, padding=(1, 2), box=ROUNDED
            )
        )
    ...
    def statistics_display(self, state: TrainerState) -> None:
        ...
        # Create stats table
        stats = Table(title="📊 Statistiques", border_style=self.theme["primary"], box=ROUNDED)
        ...
        stats.add_row("🎯 Défis quotidiens", f"{completed_challenges}/{total_challenges}")
    ...
    def daily_challenge_display(self, challenge: DailyChallenge) -> None:
        ...
        status = "[bold green]COMPLÉTÉ[/]" if challenge.completed else "[bold yellow]EN COURS[/]"
        ...
        content.add_row(f"[bold {self.theme['success']}]⭐ Récompense: {challenge.xp_reward} XP[/]")
    ...
    def achievements_display(self, achievements: List[str]) -> None:
        """Display user achievements."""
        if not achievements:
            self.console.print(
                Panel(
                    "[italic]Aucun succès débloqué pour le moment[/]",
                    border_style=self.theme["muted"],
                    box=SQUARE,
                )
            )
        ...
        content.add_row("[bold]🏆 Succès débloqués:[/]")
        content.add_row("")
        
        for achievement in achievements:
            content.add_row(f"⭐ {achievement}")
