...
class TrainerState:
    ...
    streak: int = 0
    last_activity_date: str = field(default_factory=lambda: datetime.now().strftime("%Y-%m-%d"))
    ...
    def most_common_errors(self) -> List[Tuple[str, int]]:
        ...
        return Counter(self.error_frequency).most_common(10)
    
    def add_attempt(self, attempt: Attempt) -> None:
        """Add a new attempt and maintain history limit."""
        self.attempts.append(attempt)
        if len(self.attempts) > 300:
            self.attempts = self.attempts[-300:]
        ...
        if len(self.recent_phrases) > 20:  # Keep only last 20 phrases
            self.recent_phrases = self.recent_phrases[-20:]
    ...
    def get_notebook_by_topic(self, topic: str) -> List[NotebookEntry]:
        ...
        return [entry for entry in self.notebook if entry.topic.lower() == topic.lower()]
    ...
    def _update_streak(self) -> None:
        """Update streak counter based on activity."""
        today = datetime.now().strftime("%Y-%m-%d")
