...
from .models import TrainerState, Attempt, ReviewItem, NotebookEntry
...
# from .achievements import achievement_service  # Will be used in the app controller
...
class ExerciseService:
    ...
    def _cached_generate_exercise_prompt(level: str, focus: str, theme: str) -> str:
        """Cache exercise prompts to avoid regenerating identical prompts."""
        return PromptTemplates.get_exercise_prompt(level=level, focus=focus, theme=theme)
    ...
    def generate_exercise(state: TrainerState) -> Dict[str, Any]:
        ...
        """
        try:
            # Use cached prompt generation
            prompt = ExerciseService._cached_generate_exercise_prompt(
                state.level_name,
            ...
            # Debug logging
            error_handler.logger.debug(f"AI response type: {type(response)}")
            error_handler.logger.debug(f"AI response content: {response}")
            
            if not response:
                raise AIClientError("No response from AI")
            
            if not isinstance(response, dict):
                raise AIClientError(f"Invalid response type: {type(response)}. Expected dict.")
            
            if "paragraph_fr" not in response:
                available_keys = list(response.keys()) if isinstance(response, dict) else "N/A"
                raise AIClientError(f"Missing 'paragraph_fr' in response. Available keys: {available_keys}")
            ...
            fallback_exercises = [
                {"paragraph_fr": "Je vais au marché ce matin.", "notes": "Futur proche"},
                {"paragraph_fr": "Elle a mangé une pomme hier.", "notes": "Passé composé"},
                {"paragraph_fr": "Nous sommes en train de travailler.", "notes": "Présent continu"},
                {"paragraph_fr": "Il faut que tu viennes demain.", "notes": "Subjonctif"},
                {"paragraph_fr": "Si j'avais de l'argent, j'achèterais une voiture.", "notes": "Conditionnel"},
            ]
    ...
    def evaluate_translation(
        ...
        try:
            ...
            # Debug logging
            error_handler.logger.debug(f"Evaluation response type: {type(response)}")
            error_handler.logger.debug(f"Evaluation response content: {response}")
            ...
            required_fields = ["score", "ideal_translation", "main_error", "lesson", "improvement_suggestions"]
            for field in required_fields:
                if field not in response:
                    available_keys = list(response.keys()) if isinstance(response, dict) else "N/A"
                    raise AIClientError(f"Missing '{field}' in response. Available keys: {available_keys}")
            ...
            # Fallback: try with a simpler prompt
            try:
                simple_prompt = f'Evaluate this translation from French to English. Respond with JSON: {{"score": 7, "ideal_translation": "your ideal translation", "main_error": "main issue", "lesson": "grammar tip", "improvement_suggestions": ["Suggestion 1", "Suggestion 2"]}}'
                fallback_response = ai_client.call_json(
                    system=simple_prompt,
                ...
                if fallback_response and "score" in fallback_response:
                    ...
                    fallback_response["score"] = clamp_int(fallback_response.get("score", 0), 0, 10, 0)
                    ...
                    fallback_response.setdefault("improvement_suggestions", ["Améliorez votre grammaire", "Pratiquez davantage"])
            ...
            return {
                ...
                "improvement_suggestions": ["Vérifiez votre connexion Internet", "Essayez à nouveau plus tard"]
            }
...
class LessonService:
    ...
    def generate_lesson(topic: str, level: str, settings) -> str:
        ...
        try:
            ...
            raise AIClientError(f"Failed to generate lesson: {e}")
    ...
    def answer_question(
        ...
        try:
            ...
            raise AIClientError(f"Failed to answer question: {e}")
...
class NotebookService:
    ...
    def toggle_favorite(
        ...
        if 0 <= entry_index < len(state.notebook):
            state.notebook[entry_index].favorite = not state.notebook[entry_index].favorite
            storage.save_state(state)
            return True
        return False
...
class ConversationService:
    ...
    def start_conversation(
        ...
        try:
            ...
            raise AIClientError(f"Failed to start conversation: {e}")
    ...
    def continue_conversation(
        ...
        try:
            ...
            raise AIClientError(f"Failed to continue conversation: {e}")
...
class VocabularyService:
    ...
    def generate_vocabulary_set(
        ...
        try:
            ...
            raise AIClientError(f"Failed to generate vocabulary: {e}")
    ...
    def get_daily_challenge(date_str: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = ai_client.call_json(
                ...
                temperature=0.7,
                model="gpt-4o-mini"  # Use a consistent model for challenges
            )
            ...
            return {
                ...
                "tips": ["Concentrez-vous sur le temps verbal", "Pensez aux formules de politesse"],
                "xp_reward": 10
            }
